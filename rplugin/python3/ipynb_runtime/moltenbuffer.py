...
from ipynb_runtime.position import Position
from ipynb_runtime.utils import notify_error, notify_info, notify_warn
from ipynb_runtime.outputbuffer import OutputBuffer
from ipynb_runtime.outputchunks import ImageOutputChunk, OutputChunk, OutputStatus
from ipynb_runtime.runtime import JupyterRuntime


class IpynbKernel:
    """Handles a Single Kernel that can be attached to multiple buffers
    Other IpynbKernels can be attached to the same buffers"""
    ...
    kernel_id: str
    """name unique to this specific jupyter runtime. Only used within Ipynb. Human Readable"""
    ...
    def remove_nvim_buffer(self, buffer_number: int) -> None:
        """Detach a Neovim buffer without stopping this shared kernel."""
        self.buffers = [buffer for buffer in self.buffers if buffer.number != buffer_number]

    def deinit(self) -> None:
        self._doautocmd("IpynbDeinitPre")
        self.runtime.deinit()
        self._doautocmd("IpynbDeinitPost")

    def interrupt(self) -> None:
        self.runtime.interrupt()

    def restart(self, delete_outputs: bool = False) -> None:
        if getattr(self.runtime, "external_kernel", False):
            # An externally owned kernel cannot be restarted safely here. Let the
            # runtime report that before changing the local output state.
        ...
        for chunk in output.chunks:
            if isinstance(chunk, ImageOutputChunk):
                try:
                    ...
                    img.show(f"[{output.execution_count}] Ipynb Image")
                    if not silent:
    ...
    def clear_interface(self, buffer_number: Optional[int] = None) -> None:
        if self.updating_interface:
            return

        for buffer in self.buffers:
            if buffer_number is not None and buffer.number != buffer_number:
                continue
    ...
    def clear_open_output_windows(self, buffer_number: Optional[int] = None) -> None:
        for cell, output in self.outputs.items():
            if buffer_number is None or cell.bufno == buffer_number:
                output.clear_float_win()

    def clear_virt_outputs(self, buffer_number: Optional[int] = None) -> None:
        for cell, output in self.outputs.items():
            if buffer_number is None or cell.bufno == buffer_number:
    ...
    def _delete_cell(self, cell: CodeCell, quiet=False) -> bool:
        ...
        if cell in self.outputs and self.outputs[cell].output.status == OutputStatus.RUNNING:
            if not quiet:
                notify_warn(
                    self.nvim,
                    "Cannot delete a running cell. Wait for it to finish or use :IpynbInterrupt before creating an overlapping cell.",
                )
            return False
    ...
    def update_interface(self) -> None:
        ...
        selection_changed = self.selected_cell != new_selected_cell

        # Clear the cell we just left
        if selection_changed and self.selected_cell is not None:
        ...
        render_selected = self.selected_cell is not None
