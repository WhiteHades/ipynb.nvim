...
from ipynb_runtime.utils import notify_warn, IpynbException
...
class WeztermCanvas(Canvas):
    ...
    def init(self) -> None:
        self.nvim.exec_lua("_ipynb_wezterm = require('ipynb.molten_load_wezterm_nvim').wezterm_api")
        self.wezterm_api = self.nvim.lua._ipynb_wezterm
        self.initial_pane_id = self.wezterm_api.get_pane_id()

    def deinit(self) -> None:
        """Closes the terminal split that was opened with IpynbInit"""
    ...
    def wezterm_split(self):
        ...
        """
        self.image_pane = self.wezterm_api.wezterm_ipynb_init(
...
class SnacksCanvas(Canvas):
    ...
    def init(self) -> None:
        self.nvim.exec_lua("_ipynb_snacks = require('ipynb.molten_load_snacks_nvim').snacks_api")
        self.snacks_api = self.nvim.lua._ipynb_snacks
...
def get_canvas_given_provider(nvim: Nvim, options: IpynbOptions) -> Canvas:
    ...
    if name == "none":
        ...
        if options.auto_open_output:
            raise IpynbException(
                "'wezterm' as an image provider does not currently support ipynb_auto_open_output = true, please set it to false or use a different image provider"
            )
