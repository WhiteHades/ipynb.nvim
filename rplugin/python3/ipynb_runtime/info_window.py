...
def create_info_window(nvim, ipynb_kernels, buffers, initialized):
    ...
    other_buf_kernels = set(ipynb_kernels.keys()) - set(map(lambda x: x.kernel_id, buf_kernels))
    other_kernels = set(kernel_info.keys()) - set(ipynb_kernels.keys())
    ...
    if len(other_buf_kernels) > 0:
        ...
        for kernel_id in other_buf_kernels:
            m_kernel = ipynb_kernels[kernel_id]
            running_buffers = map(lambda x: str(x.number), m_kernel.buffers)
            running = f"(running, bufnr: [{', '.join(running_buffers)}])"
            spec = m_kernel.runtime.kernel_manager.kernel_spec
            draw_kernel_info(
