...
---show the IpynbInit prompt with the given kernels
...
M.prompt_init = function(kernels, prompt)
  vim.schedule_wrap(function()
    vim.ui.select(kernels, {
      ...
      vim.schedule_wrap(function()
        if choice[2] then
          vim.cmd("IpynbInit shared " .. choice[1])
        else
          vim.cmd("IpynbInit " .. choice[1])
        end
      end)()
    end)
  end)()
...
M.prompt_stdin = function(kernel_id, prompt)
  vim.ui.input({ prompt = prompt }, function(input)
    vim.schedule(function()
      vim.fn.IpynbSendStdin(kernel_id, input)
    end)
  end)
end
