...
if not ok then
  vim.api.nvim_echo({ { "[Ipynb] `wezterm.nvim` not found" } }, true, { err = true })
  return
end
...
local validate_split_dir = function(direction)
  ...
  if not vim.tbl_contains(accepted_dirs, direction) then
    vim.notify(
      "[Ipynb] 'ipynb_split_dir' must be one of 'top', 'bottom', 'left', or 'right', defaulting to 'right'",
      vim.log.levels.WARN
    )
    return "right"
  end
...
local validate_split_size = function(size)
  if size == nil or size < 0 or size > 100 then
    vim.notify(
      "[Ipynb] 'ipynb_split_size' must be a number between 0 and 100, defaulting to a 40% split.",
      vim.log.levels.WARN
    )
    return 40
  end
...
wezterm_api.wezterm_ipynb_init = function(initial_pane_id, direction, size)
